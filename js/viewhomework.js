let prefixInfo = "info-";
let postMode = "mode=advanced&advancedMode=default";
	...
	function display_data() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				document.getElementById("listhere").innerHTML = newtext;
			}
		}
...
function modeDefault() {
	postMode = "mode=advanced&advancedMode=default";
	
	loadList(1);
}

function modeSubject() {
	const subjectId = document.getElementById("subjectId").value;
	postMode = "mode=advanced&advancedMode=subject&subjectId=" + subjectId;
	
	loadList(1);
}

function modeSent() {
	postMode = "mode=advanced&advancedMode=sent";
	
	loadList(1);
}

function modeUnsend() {
	postMode = "mode=advanced&advancedMode=unsend";
	
	loadList(1);
}

function modeLate() {
	postMode = "mode=advanced&advancedMode=late";
	
	loadList(1);
}

function modeThisweek() {
	postMode = "mode=advanced&advancedMode=week&week=this";
	
	loadList(1);
}

function modeLastweek() {
	postMode = "mode=advanced&advancedMode=week&week=last";
	
	loadList(1);
}

function modeAssign() {
	const assignStart = Date.parse( document.getElementById("assignStart").value.replace("T", " ") )/1000;
	const assignEnd = Date.parse( document.getElementById("assignEnd").value.replace("T", " ") )/1000;
	postMode = "mode=advanced&advancedMode=assign&start=" + encodeURI(assignStart) + "&end=" + encodeURI(assignEnd);
	
	loadList(1);
}

function modeDue() {
	const dueStart = Date.parse( document.getElementById("dueStart").value.replace("T", " ") )/1000;
	const dueEnd = Date.parse( document.getElementById("dueEnd").value.replace("T", " ") )/1000;
	postMode = "mode=advanced&advancedMode=due&start=" + encodeURI(dueStart) + "&end=" + encodeURI(dueEnd);
...
function goEdit() {
	if (currentViewHwid == 0) {
		bsalert("danger", '<b>แจ้งเตือน</b> ไม่สามารถแก้ไขข้อมูลได้ได้เนื่องจากไม่ได้รับข้อมูลที่เพียงพอ');		
	} else {
		showloading();
		document.getElementById("formEditId").value = currentViewHwid;
...
function goDelete() {
	if (currentViewHwid == 0) {
		bsalert("danger", '<b>แจ้งเตือน</b> ไม่สามารถลบข้อมูลได้เนื่องจากไม่ได้รับข้อมูลที่เพียงพอ');		
	} else {
		showloading();
		document.getElementById("formDeleteId").value = currentViewHwid;
...
function offList() {
	...
	document.getElementById("listhere").innerHTML = '<center class="pt-1"><img src="wheel.svg" width="24" height="24" /></center>';
}
